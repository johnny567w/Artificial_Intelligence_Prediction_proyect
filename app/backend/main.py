...
# --------------------------------------------------------------------------------------
# Paths (IMPORTANTE: PROJECT_ROOT PRIMERO)
# --------------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------------
# MLflow (DESPUÉS de PROJECT_ROOT)
# --------------------------------------------------------------------------------------
...
def _get_registry_info():
    ...
    try:
        ...
        prod_sorted = sorted(prod, key=lambda x: int(x.version), reverse=True) if prod else []
        ...
        # Si no hay production, mostramos latest version
        all_sorted = sorted(versions, key=lambda x: int(x.version), reverse=True) if versions else []
        if all_sorted:
            mv = all_sorted[0]
            return {
                "tracking_db": str(MLFLOW_DB),
...
def health():
    """
    Devuelve info del modelo activo.
    predictor.get_active_info() puede o no existir.
    Si no existe, devolvemos datos mínimos.
    """
    info = {}
    if hasattr(predictor, "get_active_info") and callable(getattr(predictor, "get_active_info")):
        try:
            info = predictor.get_active_info()
        except Exception as e:
            info = {"error": str(e)}
...
async def predict(
    ...
    # Ajusta si tu predictor usa otra firma
    result = predictor.predict_bytes(content, score_threshold=score_threshold)
...
async def predict_multi(
    ...
    for img in images:
        ...
        r = predictor.predict_bytes(content, score_threshold=score_threshold)
...
async def new_data(
    ...
    write_app_log(f"/new-data saved image={img_path.name} label={lbl_path.name}")
...
def reload_model():
    ...
    if hasattr(predictor, "reload") and callable(getattr(predictor, "reload")):
        predictor.reload()
    else:
        # tu Predictor ya tiene _load_latest, pero es privado; igual lo intentamos
        if hasattr(predictor, "_load_latest") and callable(getattr(predictor, "_load_latest")):
