...
onRetrain(): void {
  ...
  this.api.retrain().subscribe({
    next: (r) => {
      ...
      // Si el notebook se ejecutó correctamente, recargamos el modelo
      // (si tu notebook 05 promovió un nuevo Production, el predictor leerá el último best_*.pt local)
      if (r?.status === "DONE") {
  ...
  this.api.getRetrainProgress(80).subscribe({
    next: (r) => {
      if (r?.log) {
        this.logs = r.log; // o usa una variable retrainLogs
      }
    },
    error: () => {}
  });
